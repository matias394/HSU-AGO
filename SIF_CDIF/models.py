...
class CDIF_PreAdmision (models.Model):
    ...
    observaciones = models.CharField(max_length=300, null=True, blank=True)
    # Paso 2 Guarda 
    medida_abrigo_primera_vez = models.BooleanField(default=False)
    medida_abrigo_renovado = models.BooleanField(default=False)
    observaciones_guarda = models.TextField(null=True, blank=True, verbose_name="Observaciones (motivos) de la medida/guarda")
    dni_con_domicilio_san_miguel = models.CharField(max_length=100, choices=CHOICE_NOSI, null=True, blank=True, verbose_name="DNI con domicilio en San Miguel")
    padece_enfermedad_psiquica = models.CharField(max_length=100, choices=CHOICE_NOSI, null=True, blank=True, verbose_name="¿Padece alguna enfermedad psiquica?")
    cual_enfermedad = models.CharField(max_length=100, choices=CHOICE_ENFERMEDADES_MENTALES, null=True, blank=True, verbose_name="¿Cual?")
    en_tratamiento = models.CharField(max_length=100, choices=CHOICE_NOSI, null=True, blank=True,verbose_name="¿En tratamiento?")
    donde_tratamiento = models.CharField(max_length=100,  null=True, blank=True,verbose_name="¿Donde?")
    capacidades_reducidas = models.CharField(max_length=100, choices=CHOICE_NOSI, null=True, blank=True,verbose_name="¿Capacidades reducidas o afectadas o CUD?")
    cual_capacidades = models.CharField(max_length=100, null=True, blank=True, verbose_name="¿Cual?")
    centro_salud_referencia = models.CharField(max_length=100, choices=CHOICE_CENTROS_SALUD, null=True, blank=True, verbose_name="¿Centro de salud de referencia? ")
    emb_actualmente_guarda =  models.CharField(verbose_name='¿Esta embarazada actualmente?',max_length=50, choices=CHOICE_SINO, null=True, blank=True)
    controles_guarda =  models.CharField(verbose_name="¿Tiene los controles?",max_length=50, choices=CHOICE_SINOINSU, null=True, blank=True)
    emb_actual_guarda =  models.CharField(verbose_name="Embarazo actual",max_length=150, choices=CHOICE_EMB_RIESGO, null=True, blank=True)
